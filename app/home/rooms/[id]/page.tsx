    ...
    <div>
      ...
      {cornerDef && (
        <div className={`mb-8 transition-all duration-700 ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div ref={linesRef} className="space-y-1 border-l-2 border-warm-200 pl-4">
            {lines.map((line: string, i: number) => (
              <p
                key={i}
                className={`text-sm italic leading-relaxed text-warm-400 transition-all duration-500`}
                style={{
                  opacity: entered ? 1 : 0,
